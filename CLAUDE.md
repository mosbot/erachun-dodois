...
docker compose exec web python scripts/reconcile_planfact.py
docker compose exec web python scripts/reconcile_planfact.py --apply
```

The `reconcile_planfact.py` step is not optional and must run before anyone opens the portal:
the `Invoice` model maps four `planfact_*` columns, so every `session.query(Invoice)` raises
`UndefinedColumn` against a database that hasn't had them added yet. Run without `--apply`
first to preview the reconcile (it will report expected counts), then run with `--apply` to
commit. The script adds the columns even if `planfact.api_key` isn't configured on the server
yet — see the ordering rationale in the script's own docstring.
...
13. **Half-cent rounding must be HALF_UP, not Python's float `round()`** — Dodois is .NET and its server-side validation of `pricePerUnitWithVat = pricePerUnitWithoutVat × (1 + taxRate)` rounds half-cents UP. Python's `round(1.075, 2)` returns `1.07` because 1.075 is stored as 1.0749999…; banker's rounding plus float imprecision pushes it the wrong way. Invoice `2810-1-1` (Jana/Jamnica beverages) was rejected with `SUPPLY_ITEM_PRICE_PER_UNIT_WITH_VAT_WRONG_CALCULATION` for three items (Expected 1.08 vs Actual 1.07, etc.). Fix: `dodois_uploader._round2` uses `Decimal.quantize(ROUND_HALF_UP)`, `_compute_price_per_unit` runs in Decimal, and `build_supply_payload` derives `pricePerUnitWithVat` from `pricePerUnitWithoutVat × (1 + taxRate)` to exactly mirror the server's formula (independent of whatever tax amount the XML reports).
